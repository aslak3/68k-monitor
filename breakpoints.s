...
listbreakpoints:movem.l %d0-%d1/%a1-%a2,-(%sp)
		movea.l #bplist,%a1		| get start of breakpoint list
...
settraps:	movem.l %d1/%a1-%a2,-(%sp)
		movea.l #bplist,%a1		| get start of breakpoint list
		move.w #BP_COUNT-1,%d1		| 8 breakpoints to check
1:		movea.l (BP_ADDRESS,%a1),%a2	| get the breakpoint address
		tst.l %a2			| bp not in use?
		beq 2f				| skip it
		move.w (%a2),(BP_WORD,%a1)	| save original word
		cmpa.l resumepc,%a2		| is this the resume pc?
		beq 2f				| skip writing trap if so
		move.w #TRAP_INSTR,(%a2)	| write TRAP instruction
2:		add.l #BP_SIZE,%a1		| next bp entry
		dbra %d1,1b			| done them all?
		movem.l (%sp)+,%d1/%a1-%a2
		rts
...
cleartraps:	movem.l %d1/%a1-%a2,-(%sp)
		movea.l #bplist,%a1		| get start of breakpoint list
		move.w #BP_COUNT-1,%d1		| 8 breakpoints to check
1:		movea.l (BP_ADDRESS,%a1),%a2	| get the breakpoint address
		tst.l %a2			| bp not in use?
...
resumepc:	.space 4			| space to save pc to use when resuming
